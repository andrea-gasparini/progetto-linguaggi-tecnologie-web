...
class GroupsController extends \chriskacerguis\RestServer\RestController
{
	...
	public function sendMessageToGroupChat_post() {
		...
		if($tokenData["status"]) {
			...
			if($messageId > 0)
				return $this->response(buildServerResponse(true, "Messaggio inviato.", array("user_id" => $userId, "message" => $chatMessage, "username" => $user[0]->username, "picture" => $user[0]->profile_picture, "date" => date("Y-m-d H:i:s"), "ismine" => "t", "id" => $messageId, "chatToken" => $chatToken)), 200);
